...
**費用: 月 $0.3〜1**（読み取り1件 $0.005 × 1日2〜6件）。月額の最低料金はありません。
...
| **返信すべて**（交流リプ・導線リプとも） | **取らない**（`exclude=retweets,replies`） |
| **リポスト** | **取らない** |

### なぜ返信を全部切ったか（2026-08-15の判断）

初回実行で、**交流リプが1日50件**ありました。APIには「自分への返信だけ残す」
指定が無いため、導線リプを取ろうとすると交流リプも全部ついてきます。
...
**捨てるものに月$7.5は見合わない**ので、返信ごと切りました。

**導線の実測は月次のエクスポートCSVで拾えます。** あちらには全投稿の
`URLクリック数` が入っており、19ケースのファネル分析もそこから出したものです。
**日次で取れないだけで、測れなくなるわけではありません。**

特定の日の導線だけ見たいときは `--with-replies` を付けて実行します。
...
環境の**セットアップスクリプト**欄に、次の1行を足してください。

```bash
pip install -q requests requests-oauthlib
```

**これが無いとRoutineは毎回失敗します。** セッションのコンテナは毎回
まっさらで、`requests-oauthlib` は標準で入っていません（`pandas` は入っています）。

既に `npm install` などが書かれている場合は、**消さずに下へ足してください。**
...
**既にある行は消さないでください。** この欄は `.env` 形式のテキストエリア1つで、
書き換えると他のプロジェクトの設定（`JQUANTS_API_KEY` など）が消えます。
**末尾に追加**します。

```
JQUANTS_API_KEY=...          ← 既存。そのまま残す
X_API_KEY=（コンシューマーキー）
X_API_SECRET=（コンシューマーシークレット）
X_ACCESS_TOKEN=（アクセストークン）
X_ACCESS_SECRET=（アクセストークンシークレット）
