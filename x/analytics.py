...
FORMAT_RULES = [
    ...
    # 型11 箇条書きリスト（グループF）。箇条書きが6つ以上で、総括の接続が無い形。
    # 型2のリスト形式もここに入る（同じ骨格なので、まとめて数えたほうが実態に合う）
    ("型11 箇条書きリスト", lambda t: len(re.findall(r"^[・･]", str(t), re.M)) >= 6
     and not re.search(r"(などあるが|など基本だが|これだけでも|など[^。\n]{0,12}(総じて|とくに|特に|実は))", str(t))),
    # 型7 は 2026-08-31 に廃止。過去の1本を正しく分類するためだけに残しています
    # （新規には書きません）。箇条書きの中のセリフを会話劇と誤判定しないこと
    ("型7 会話だけ（廃止）", lambda t: len(re.findall(r"^(?![・･]).{0,8}「", t, re.M)) >= 3),
